...
def encode_description(df: pd.DataFrame) -> pd.DataFrame:
    encode_description_precomputed()
    
def encode_description_precomputed(df: pd.DataFrame) -> pd.DataFrame:
    precomputed_path = Path("../data/features.csv")
    if not precomputed_path.exists():
        ...
        df_unraveled.columns = [
            f"Dimension_{i+1}" for i in range(df_unraveled.shape[1])
        ]
