...
from product_bundle.association_rules import rule_based_product_bundle
from product_bundle.collaborative_filtering import \
    collaborative_filtering_product_bundle
from product_bundle.content_based_filtering import \
    content_filtering_product_bundle
from product_bundle.utils import (load_product_df, save_best_params,
                                  save_bundles)


def evaluate_bundles(df: pd.DataFrame, predicted_bundles: Set[FrozenSet], verbose=0):
    actual_bundles_df = df.groupby("InvoiceNo")["StockCode"].apply(frozenset)
    actual_bundles = set(actual_bundles_df[actual_bundles_df.apply(len) > 1].to_list())
