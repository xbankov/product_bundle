...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    logging.debug("Impute missing CustomerID rows with Unknown")
    df["CustomerID"].fillna("Unknown")
    ...
    logging.debug("Removing special StockCodes:")
...
def resolve_cancellations(df: pd.DataFrame) -> pd.DataFrame:
    ...
    cancellations = cancellations.loc[
        :, ["StockCode", "CustomerID", "InvoiceDate", "Quantity"]
    ]
